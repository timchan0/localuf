...
@pytest.mark.parametrize("gate_noise", [
    'standard',
    'balanced',
    'ion trap',
])
def test_init(gate_noise):
    edge_dict: dict[EdgeType, tuple[Edge, ...]] = {'S': (((0, 0, 0), (1, 0, 0)),)}
    with mock.patch(
        "localuf.error_models.CircuitLevel._make_multiplicities",
    ...
    assert cl._COEFFICIENTS == CircuitLevel._ALL_COEFFICIENTS[gate_noise]
...
def test_make_error(toy_cl: CircuitLevel, e_westmost: tuple[Edge, Edge]):
    ...
    for p in (0, 1):
        with mock.patch(
            'localuf.error_models.CircuitLevel._get_bitflip_probabilities',
...
def test_get_bitflip_probabilities(cl: CircuitLevel):
    with mock.patch('localuf.error_models.main._MultisetHandler.pr') as mock_pr:
...
def test_get_edge_probabilities(cl: CircuitLevel):
    p = 1e-1
    with mock.patch("localuf.error_models.CircuitLevel.get_edge_probabilities") as mock_gep:
