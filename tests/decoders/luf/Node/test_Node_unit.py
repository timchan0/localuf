...
def n3_with_E(n3: _Node):
    """Node (with correct accessibles) and a pointer string."""
    pointer = 'E'
    e, index = n3.NEIGHBORS[pointer]
    n3.accessibles = {pointer: n3.NODES.dc[e[index]]}
    return n3, pointer
...
def test_neighbors_attribute(ufn: _Node, v00, v000):
    ...
    em = ufn.NODES.LUF.CODE.ERROR_MODEL
    if str(em) == 'code capacity':
        ...
        raise ValueError(f"Unknown error model {em}")
...
def test_accessibles_attribute(ufn3: _Node):
    assert type(ufn3.accessibles) is dict
    assert ufn3.accessibles == {}
...
def update_accessibles_helper(luf: LUF, wce):
    ...
    center.update_accessibles()
    assert center.accessibles == {'W': luf.NODES.dc[w]}
    luf.growth[c, e] = Growth.FULL
    center.update_accessibles()
    assert center.accessibles == {
        'W': luf.NODES.dc[w],
        'E': luf.NODES.dc[e],
    }
    luf.growth[w, c] = Growth.UNGROWN
    center.update_accessibles()
    assert center.accessibles == {'E': luf.NODES.dc[e]}
...
def test_syncing(n3_with_E: tuple[_Node, direction]):
    ...
    vn3.accessibles[pointer].active = True
    vn3.syncing()
    assert vn3.busy
