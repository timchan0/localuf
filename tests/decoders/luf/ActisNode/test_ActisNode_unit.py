...
from localuf.decoders.luf import LUF, ActisNodes, ActisNode, NodeFriendship
...
def in3_with_E(in3: ActisNode):
    """ActisNode (with correct accessibles) and a pointer string."""
    pointer = 'E'
    e, index = in3.NEIGHBORS[pointer]
    in3.accessibles = {pointer: in3.NODES.dc[e[index]]}
    return in3, pointer

def test_init(in_: ActisNode):

    d = in_.NODES.LUF.CODE.D
    em = in_.NODES.LUF.CODE.ERROR_MODEL
...
def test_countdown_start(actis_nodes: ActisNodes):
    ...
    if str(actis_nodes.LUF.CODE.ERROR_MODEL) == 'code capacity':
        for cs in range(2*d):
            if cs < d:
                assert ct[cs] == cs + 1
            else:
                assert ct[cs] == 2*d - cs
    elif str(actis_nodes.LUF.CODE.ERROR_MODEL) == 'phenomenological':
        # cs goes up somewhat as triangular numbers
        pass
    elif str(actis_nodes.LUF.CODE.ERROR_MODEL) == 'circuit-level':
